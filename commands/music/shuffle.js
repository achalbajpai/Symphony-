    ...
    run: async (client, message, args) => {
     ...
     .setTimestamp();
      ...
      try {
       ...
       let stop = queue.shuffle();
       let pauseEm = new MessageEmbed()
       .setAuthor('Music Shuffled', client.user.displayAvatarURL({dynamic: true}))
       .setColor('#4ae7ff')
       .setDescription(`The music queue is now well mixed.`)
       .setTimestamp();
    message.channel.send({embeds: [pauseEm]});
      } catch (e) {
        let errorEm = new MessageEmbed()
        .setTitle('Error Occured in Shuffle Command')
        .setDescription(e)
