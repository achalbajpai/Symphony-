    ...
    run: async (client, message, args) => {
     ...
     .setTimestamp();
      ...
      try {
       ...
       .setAuthor('Stopped!', client.user.displayAvatarURL({dynamic: true}))
       .setColor('#4ae7ff')
       .setDescription(`Stopped the music. Use play command to again start music.`)
       .setTimestamp();
    message.channel.send({embeds: [pauseEm]});
      } catch (e) {
        let errorEm = new MessageEmbed()
        .setTitle('Error Occured in Stop Command')
        .setDescription(e)
