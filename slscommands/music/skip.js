...
module.exports = {
    name: "skip",
    description: "skips the currently played music!",
    permissions : [""],
    /**
     *
     * @param {Client} client
    ...
    run: async (client, interaction, args) => {
     ...
     const ch = interaction.channel;
      ...
      try {
       ...
       .setAuthor('Skippped!', client.user.displayAvatarURL({dynamic: true}))
       .setColor('#4ae7ff')
       .setDescription(`Now playing the next music in queue!`)
       .setTimestamp();
       interaction.followUp({embeds: [pauseEm]});

      } catch (error) {
        let errorEm = new MessageEmbed()
        .setTitle('Error Occured in Skip Command')
